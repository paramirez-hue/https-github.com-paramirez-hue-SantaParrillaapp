...
const App: React.FC = () => {
  ...
  const handlePayment = async () => {
    ...
    setIsPaying(true);
    try {
      const displayLocation = orderType === 'table' ? `Mesa ${tableNumber}` : `Domicilio: ${tableNumber}`;
      const newOrder = { 
        items: cart, 
        total: cartTotal, 
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-sans bg-[#F8F9FA] text-slate-900">
                    ...
                    <div className="p-8 pb-6 border-b border-dashed flex justify-between items-start">
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span className={`${order.tableNumber?.toLowerCase().includes('domicilio') ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-slate-50 text-slate-400 border-slate-100'} border font-mono text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-xl`}>
                            {order.tableNumber}
                          </span>
                        </div>
      ...
      {isCartOpen && <CartView cart={cart} setCart={setOrderItems} customerName={customerName} setCustomerName={setCustomerName} tableNumber={tableNumber} setTableNumber={setTableNumber} orderType={orderType} setOrderType={setOrderType} cartTotal={cartTotal} isPaying={isPaying} paymentSuccess={paymentSuccess} handlePayment={handlePayment} trackedOrder={trackedOrder} setShowTrackingView={setShowTrackingView} onClose={() => setIsCartOpen(false)} />}
...
const CartView = ({ cart, setCart, customerName, setCustomerName, tableNumber, setTableNumber, orderType, setOrderType, cartTotal, isPaying, paymentSuccess, handlePayment, trackedOrder, setShowTrackingView, onClose }: any) => {
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
         <div className="p-6 md:p-8 border-b flex justify-between items-center pt-safe">
            ...
            <div className="flex items-center gap-2">
              {trackedOrder && (
                <button 
                  onClick={() => { setShowTrackingView(true); onClose(); }} 
                  className="flex items-center gap-2 px-3 py-2 bg-orange-50 text-orange-600 rounded-xl font-black text-[9px] uppercase border border-orange-100 hover:bg-orange-100 transition-all"
                >
                  <Timer className="w-3.5 h-3.5" /> Ver Pedido Activo
                </button>
              )}
            ...
            {cart.length > 0 && (
              <div className="pt-4 space-y-6">
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-4">Tipo de Pedido</label>
                  <div className="flex p-1.5 bg-slate-100 rounded-[2rem] border border-slate-200">
                    <button 
                      onClick={() => setOrderType('table')} 
                      className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[1.5rem] font-black text-[10px] uppercase transition-all ${orderType === 'table' ? 'bg-white text-slate-900 shadow-premium' : 'text-slate-400 opacity-60'}`}
                    >
                      <UtensilsCrossed className="w-4 h-4" /> A la Mesa
                    </button>
                    <button 
                      onClick={() => setOrderType('delivery')} 
                      className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-[1.5rem] font-black text-[10px] uppercase transition-all ${orderType === 'delivery' ? 'bg-orange-600 text-white shadow-orange-glow' : 'text-slate-400 opacity-60'}`}
                    >
                      <ShoppingBag className="w-4 h-4" /> A Domicilio
                    </button>
                  </div>
                </div>
                ...
                <div className="space-y-4 pt-2">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">Información del Cliente</label>
                    <input type="text" placeholder="Tu Nombre Completo" value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full p-4.5 bg-slate-50 rounded-2xl outline-none font-black text-[11px] uppercase border border-slate-100 focus:border-orange-500 shadow-inner" />
                  </div>
                  
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-4">
                      {orderType === 'table' ? 'Número de Mesa' : 'Dirección de Entrega'}
                    </label>
                    <input 
                      type="text" 
                      placeholder={orderType === 'table' ? "Ej: Mesa 5" : "Calle, número y barrio"} 
                      value={tableNumber} 
                      onChange={e => setTableNumber(e.target.value)} 
                      className="w-full p-4.5 bg-slate-50 rounded-2xl outline-none font-black text-[11px] uppercase border border-slate-100 focus:border-orange-500 shadow-inner" 
                    />
                  </div>
                </div>
              </div>
