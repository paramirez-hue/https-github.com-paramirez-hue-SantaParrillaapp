        ...
        <div className="fixed inset-0 z-[200] flex justify-end">
          ...
          <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-500">
             ...
             <div className="p-10 border-t bg-white pb-safe">
                ...
                <button onClick={handlePayment} disabled={cart.length === 0 || isPaying || !customerName} className={`w-full py-6 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] shadow-xl active:scale-95 transition-all ${paymentSuccess ? 'bg-emerald-500 text-white' : 'bg-slate-950 text-white disabled:opacity-20'}`}>{isPaying ? 'Procesando...' : paymentSuccess ? '¡Enviado!' : 'Hacer Pedido'}</button>
             </div>
          </div>
        </div>
      )}
